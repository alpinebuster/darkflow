...
impl ProfilingSession {
    pub fn start_from_env(mode: &'static str) -> Result<Option<Self>> {
        let flamegraph_path = env::var_os("DARKFLOW_PROFILE_FLAMEGRAPH").map(PathBuf::from);
        let sampling_frequency_hz = env::var("DARKFLOW_PROFILE_FREQUENCY_HZ")
            .ok()
            .and_then(|value| value.parse::<i32>().ok())
            .filter(|value| *value > 0)
            .unwrap_or(DEFAULT_PROFILE_FREQUENCY_HZ);

        let enabled = flamegraph_path.is_some() || env_flag("DARKFLOW_PROFILE_RESOURCE_SUMMARY");
        if !enabled {
            return Ok(None);
        }
        ...
        info!(
            ...
            env_flag("DARKFLOW_PROFILE_RESOURCE_SUMMARY"),
