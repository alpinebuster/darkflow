...
smoke_dir="$concap_dir/darkflow-smoke"
completed_dir="$smoke_dir/completed/$scenario_name"
pcap_path="$completed_dir/dump.pcap"
output_csv="$completed_dir/darkflow-current.csv"
...
cp "$concap_dir/example/processingpods/darkflow.yaml" "$smoke_dir/processingpods/"
...
echo "Reprocessing $pcap_path with current local Darkflow"
cargo run -p darkflow -- \
    -f darkflow \
    --header \
    --idle-timeout 120 \
    --active-timeout 3600 \
...
print(f"Current Darkflow rows: {len(rows) - 1}")
print(f"Current Darkflow columns: {len(header)}")
