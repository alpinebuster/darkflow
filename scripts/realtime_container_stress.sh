...
Run the local containerized Darkflow realtime stress harness on rgbcore.

This script assumes:
- the persistent local test harness exists
- darkflow-t0 is the host-side capture interface
- an iperf3 server is already running inside darkflow-peer on 10.203.0.2:5201

Options:
  --image NAME         Docker image to run (default: darkflow:test-slim)
  --container NAME     Container name prefix (default: darkflow-stress)
  --features NAME      Flow type to export (default: darkflow)
  --interface NAME     Capture interface (default: darkflow-t0)
  --threads N          Darkflow worker threads (default: 4)
  ...
  --early-export SEC   Darkflow early-export interval in seconds
                       (default: 5, use 0 to disable)
  --env KEY=VALUE      Extra environment variable for the Darkflow container
                       (repeatable)
  --export-path PATH   CSV export path inside the host filesystem
                       (default: target/realtime-stress/darkflow-stress.csv)
...
image="darkflow:test-slim"
container_prefix="darkflow-stress"
features="darkflow"
interface="darkflow-t0"
threads=4
bitrate="2.5G"
length=1400
parallel=1
...
export_path="$repo_root/target/realtime-stress/darkflow-stress.csv"
container_env=()
...
if [[ "$early_export" != "0" ]]; then
    darkflow_args+=(--early-export "$early_export")
fi

darkflow_args+=(realtime "$interface" --ingress-only)
